...
class DiscountCalculator:
    ...
    def _apply_percentage_discount(self):
        ...
        percentage_rules = [
            rule for rule in DiscountRule.get_active_rules() 
            if rule.discount_type == 'percentage' 
            and (rule.min_order_amount is None or self.order.subtotal >= rule.min_order_amount)
        ]
        ...
        existing_percentage = self.discount_breakdown.get('percentage_discount', {}).get('amount', Decimal('0'))
    ...
    def _apply_flat_discount(self):
        ...
        flat_rules = [
            ...
            and (
                rule.min_completed_orders is None or 
                self.order.user.eligible_for_flat_discount
            )
        ]
        ...
        existing_flat = self.discount_breakdown.get('flat_discount', {}).get('amount', Decimal('0'))
        ...
        for rule in category_rules:
            if rule.category_id in category_items:
                ...
                if rule.min_quantity is None or total_quantity >= rule.min_quantity:
                    ...
                    for item in items:
                        discount_per_item = (item.unit_price * rule.value) / Decimal('100')
